// Component that is mounted when a playbook for a task should be created manually
<template>
    <div class="card">
        <div class="p-d-flex p-jc-between p-flex-column p-flex-sm-row">     
            <span class="p-input-icon-right">
                <Button @click="$router.push({ name: 'singletaskview', params: { id: id }});">Back</Button>
            </span>
        </div>
    </div>
...
<!-- task information overview -->
    ...
    <div class="p-grid button-demo">
        <div class="p-col-12 p-md-6">
            <!-- CSAF document presentation left -->
        ...
        <div class="p-col-12 p-md-6">
            <!-- Playbook Configurator right -->
            <div class="card">
                <ScrollPanel style="width: 100%; height: 850px">
                    <div>
                        <PlaybookConfigurator @playbook-created="createPlaybook"></PlaybookConfigurator>
                    </div>
                </ScrollPanel>
            </div>
        </div>
...
export default {
    ...
    computed: {
        ...
        task() {
            return this.$store.getters['task/getById'](this.taskId);
        }
    },
    methods: {
        async fetchData() {
        ...
        async createPlaybook(data) { //method called when the configurator emits the data of the playbook and saves it to the database
            try {
                await this.$store.dispatch('playbook/create', data); // creates playbook
                const pbId = this.$store.getters['playbook/getRecentCreated']._id;
                const taskToUpdate = this.task;
                
                taskToUpdate.playbook.push(pbId);
                taskToUpdate.modified = new Date().toISOString();
                await this.$store.dispatch('task/update', taskToUpdate); // adds the created playbook to the existing task

                this.$router.push({ name: 'singletaskview', params: { id: this.taskId }}).then(() => {
                        this.$toast.add({ severity: 'success', summary: 'Playbook created!', life: 2000 });
                });
            }catch(error) {
                this.$toast.add({severity:'error', summary: 'Error while creating playbook!', life: 2000});
            }
        }
    },
    created() {
